...
class ImageConditionedObjectDetector:
    ...
    def process(self, img_dir):
        ...
        print(f"Inference time: {time.time()-t0:.3f}")

        # # Convert predictions to numpy
        # predictions = jax.tree_util.tree_map(lambda x: np.array(x[0]), predictions)

        # # Extracting scores, boxes, and labels from predictions
        # logits = predictions['pred_logits'][..., :len(tokenized_queries)]
        # scores = sigmoid(np.max(logits, axis=-1))
        # labels = np.argmax(predictions['pred_logits'], axis=-1)
        # boxes = predictions['pred_boxes']
        # objectnesses = sigmoid(predictions['objectness_logits'])
        # class_embeddings = sigmoid(predictions['class_embeddings'])
    ...
    def process_with_embeddings(self, img_dir, query_embedding, class_names, conf_thresh, avg_count=1, merging_mode='average'):
        ...
        # print(averaged_queries.shape)
        # return
        target_class_predictions = self.class_predictor(
            image_features=feature_map.reshape(b, h * w, d),
            query_embeddings=averaged_queries,  # [batch, queries, d]
        )
        ...
        # scores = np.max(target_logits, axis=1)

        selected_indexes = np.where(scores>conf_thresh)[0]
        
        # selected_indexes = np.argpartition(scores, -top_k)[-top_k:]
        # # selected_indexes = [np.argmax(target_logits[:, 0], axis=0)]
        # scores = sigmoid(target_logits[selected_indexes, 0])

        # selected_bboxes = target_boxes[selected_indexes]
        ...
        if len(self.time_diffs) > 1:
            ...
            print(f"Inference time: {mean_dt:.3f}")
